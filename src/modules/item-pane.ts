    ...
    Zotero.ItemPaneManager.registerSection({
      ...
      onAsyncRender: async ({
          ...
          multiturn_btn.addEventListener("click", () => {
            ...
            if (multiTurnActive) {
              ...
              if (currentResponse && multiTurnHistory.length === 0) {
                const systemPromptLang = getPrompt('system_multi_turn', getPref('lang') as string || 'zh-CN');
        ...
        async function ask_question() {
          ...
          let system_prompt = '';
          if (associateActive) {
            system_prompt = getPrompt('ask_associate', lang);
          } else {
            system_prompt = getPrompt('ask', lang);
          }
        ...
        async function summarize_text() {
          ...
          let system_prompt = '';
          if (associateActive) {
            system_prompt = getPrompt('summarize_associate', lang);
          } else {
            system_prompt = getPrompt('summarize', lang);
          }
